...
def searchStudents(request):
    if request.method == 'POST':
        req = request.POST.dict()  # convert request to dict
        name = req['student_name']
        students = Student.objects.filter(student_name__contains=name).order_by('student_name')
    else:
        students = Student.objects.all().order_by('student_name')
    data = {
        'students': students,
    }
    return render(request, 'student/search.html', context=data)
